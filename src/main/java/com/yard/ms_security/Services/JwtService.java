...
public class JwtService {
    ...
    public String generateToken(User theUser) {
        ...
        return Jwts.builder()
                ...
                .signWith(secretKey) // verificar de lo que hice antes si lo haya hecho la persona que es
                .compact();
    }
    ...
    public User getUserFromToken(String token) {
        try {
            Jws<Claims> claimsJws = Jwts.parserBuilder()//Convierte el toquen que le acabe de mandar y le verifica la firma, devolviendo que encontro en este
                    .setSigningKey(secretKey)
                    .build()
            ...
            User user = new User();//Devuelve un usuario vacion, llenandolo con los datos que acaba de descodificar del token
            user.set_id((String) claims.get("_id"));//Codifica cada cosa una por una
            user.setName((String) claims.get("name"));
            user.setEmail((String) claims.get("email"));
            return user; //Devuelvo el usuario
        } catch (Exception e) {
